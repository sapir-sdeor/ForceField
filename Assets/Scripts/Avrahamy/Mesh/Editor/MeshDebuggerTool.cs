...
namespace Avrahamy.Meshes {
    ...
    public class MeshDebuggerTool : EditorTool {
        ...
        public override void OnToolGUI(EditorWindow window) {
            ...
            var duplicatedNormals = new List<Vector3>();
            var normals = new List<Vector3>();
            var uvs = new List<Vector2>();
            foreach (var i in indices) {
                if (i < mesh.normals.Length) {
                    var normal = meshFilter.transform.TransformVector(mesh.normals[i]).normalized;
                    if (normals.Contains(normal)) {
                        duplicatedNormals.Add(normal);
                    }
                    normals.Add(normal);
                } else {
            ...
            for (int i = 0; i < normals.Count; i++) {
                ...
                if (multipleVertices) {
                    var offset = 0.1f;
                    if (duplicatedNormals.Contains(normal)) {
                        offset += duplicates * 0.03f;
                        ++duplicates;
                    }
                    position += normal * offset * scale;
                }
                ...
                Handles.color = duplicatedNormals.Contains(normal) ? Color.red : Color.green;
